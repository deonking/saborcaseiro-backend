$(document).ready(function() {
    // Monitor quantity changes for sections
    $(document).on('click', '.adicionarQtdeOpcao', function() {
        const tipoSection = $(this).closest('.tipo');
        const maxItems = parseInt(tipoSection.data('maximo'));
        
        // Get total selected items in this section
        let total = 0;
        tipoSection.find('.qtdeOpcao').each(function() {
            total += parseInt($(this).val() || 0);
        });
        
        // If this is the acompanhamentos section and we've reached the max
        if (tipoSection.find('h3').text().includes('Acompanhamentos') && total >= maxItems) {
            setTimeout(() => {
                const carnesSection = document.getElementById('section-carnes');
                if (carnesSection) {
                    $('html, body').animate({
                        scrollTop: $(carnesSection).offset().top - 100
                    }, 800);
                }
            }, 300); // Small delay to ensure counter is updated first
        }
    });
	...
	$(document).on('click', '#menuCategorias .categorias a', function (e) {
		...
		// Modified scroll implementation
		let targetElement = $(aid);
